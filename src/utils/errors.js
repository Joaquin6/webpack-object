...
const filterChildren = (children) => children.filter((err) => err.keyword !== 'anyOf' && err.keyword !== 'allOf' && err.keyword !== 'oneOf')
...
class WebpackOptionsValidationError extends OptionsError {
  constructor(validationErrors, webpackOptionsSchema) {
    super()
  ...
  static formatValidationError(err) {
    ...
    if (err.keyword === 'additionalProperties') {
      ...
      if (err.params.limit === 1) { return `${dataPath} should not be empty.${getSchemaPartDescription(err.parentSchema)}` } else { return `${dataPath} ${err.message}${getSchemaPartDescription(err.parentSchema)}` }
